...
CLOSURE_BUNDLE_TEMPLATE = """# External closure bundle: {candidate_id}

Generated by `M1/orchestrator/controller.py` for the candidate focused on external closure
({pass_count} of 5 gates PASS, {auto} autonomous blocker(s) remaining, {ext} external item(s)).

State: `{gates}`

Universe rule: `{universe_rule}` (spec: `M1/hypotheses/candidate_specs/`)

Send everything below as one message per organisation. Do not send overlapping packets: the four
scoped items this bundle consolidates are {external_blockers}.
...
def closure_bundle_md(metrics_row, candidate, venue, envelope, cards) -> str:
    def fmt_fee(value, unit):
        return "UNKNOWN" if value in (None, "") else f"{value} {unit}"

    verified = [
        f"- displayed-add rebate: {fmt_fee(venue.get('rebate_value'), venue.get('rebate_unit'))}"
        f" (source {venue.get('rebate_source_id')})",
        f"- remove fee: {fmt_fee(venue.get('taker_fee_value'), venue.get('taker_fee_unit'))}"
        f" (source {venue.get('taker_fee_source_id')})",
    ...
    return CLOSURE_BUNDLE_TEMPLATE.format(
        candidate_id=metrics_row["candidate_id"],
        pass_count=metrics_row["pass_count"],
        auto=metrics_row["autonomous_distance_to_eligibility"],
        ext=metrics_row["external_distance_to_eligibility"],
        gates=f"{metrics_row['pass_gates']} PASS | {metrics_row['blocked_gates']} BLOCKED",
        universe_rule=candidate.get("universe_rule_id"),
        external_blockers=metrics_row["external_blockers"],
        verified="\n".join(verified),
        private="\n".join(private),
    )
...
def main(argv) -> int:
    ...
    if command == "status":
        ...
        print(json.dumps({"closure_modes": data.get("closure_modes"),
                          "next_autonomous_branch": data.get("next_autonomous_branch"),
